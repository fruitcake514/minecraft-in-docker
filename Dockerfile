...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    gosu passwd git unzip || rm -rf /var/lib/apt/lists/*
...
RUN ARCH=$(dpkg --print-architecture) && \
    wget -O /tmp/sunshine.deb \
      https://github.com/LizardByte/Sunshine/releases/latest/download/sunshine-ubuntu-22.04-${ARCH}.deb && \
...
# Entrypoint handles remap + launch
